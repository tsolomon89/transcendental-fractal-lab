...
const ModelControls: React.FC<ModelControlsProps> = ({ params, setParams }) => {
    ...
    return (
        <ControlGroup title="Model">
            ...
            <NumberInput label="τ Power (k)" value={model.k} onChange={v => updateModelParam('k', v)} step={0.01} precision={3} />
            {model.mode === RenderMode.Julia && (
                <div className="pl-4 border-l-2 border-gray-600 space-y-1 mt-2">
                    <NumberInput label="Julia C (Re)" value={model.juliaC.re} onChange={v => updateModelParam('juliaC', { ...model.juliaC, re: v })} />
                    <NumberInput label="Julia C (Im)" value={model.juliaC.im} onChange={v => updateModelParam('juliaC', { ...model.juliaC, im: v })} />
                </div>
            )}
             {model.type === FractalModel.B1 && (
                <div className="pl-4 border-l-2 border-cyan-500 space-y-1 mt-2">
                    ...
                    <NumberInput label="λ (Re)" value={model.b1.lambda.re} onChange={v => updateB1Param('lambda', { ...model.b1.lambda, re: v })} />
                    <NumberInput label="λ (Im)" value={model.b1.lambda.im} onChange={v => updateB1Param('lambda', { ...model.b1.lambda, im: v })} />
                </div>
            )}
                ...
                <div className="pl-4 border-l-2 border-cyan-500 space-y-2 mt-2">
                     ...
                     { [0,1,2,3].map(i => (
                         <div key={i} className="space-y-1 p-2 bg-gray-800 rounded">
                            ...
                            <NumberInput label={`α${i+1} (Re)`} value={model.b2.alpha[i].re} onChange={v => updateB2Param('alpha', i, { ...model.b2.alpha[i], re: v })} />
                            <NumberInput label={`α${i+1} (Im)`} value={model.b2.alpha[i].im} onChange={v => updateB2Param('alpha', i, { ...model.b2.alpha[i], im: v })} />
                            <NumberInput label={`M${i+1}`} value={model.b2.M[i]} onChange={v => updateB2Param('M', i, v)} step={1} precision={0} />
                         </div>
                     ))}
                </div>
            )}
